...
# DUMP_ARGS=
#  docker build -f ./Dockerfile -t adockero/mysqldump:tables-ssh-cpu --build-arg FILE_ARG=dump-import.md5.cpu.sh  .
...
while true; do echo \$(mpstat 1 1 |grep "Average:" | awk '{print \$NF}'); sleep 1; done
        ...
        for table in $tables; do
                ...
                if [[ ${ASYNC_WAIT} == "" ]]; then
                        ...
                        while true; do
                                ...
                                cpuWait=$(cat /tmp/get_remote_cpu_idle)

                                if [[ "$current_jobs" -lt "$ASYNC_WAIT_MAX" ]] && [[ "$cpuWait" > "$CPU_IDLE_MIN" ]]; then
